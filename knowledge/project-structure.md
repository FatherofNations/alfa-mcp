...
## Пресеты наполнения (готовые блоки)

Главная страница собирается мгновенно из byte-perfect блоков проверенного
прототипа: `scaffold_project` / `register_dashboard` с
`preset: "accountant"` — быстрые действия, баланс, дела в работе, баннеры,
табло с онбордингом, лента операций (классы `cur-*`, стили в
`styles/accountant.css`, ассеты в `assets/accountant/`). Можно взять
подмножество: `presetBlocks: ["quick-actions", "feed"]`. Демо-контент
правится прямо в разметке блоков (next: `accountantHtml.ts`). Пресет
рассчитан на включённый chrome.
